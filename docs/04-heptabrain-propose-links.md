...
- **v1.2.2 (2026-04-24 晚，Gemini + ChatGPT sign-off review 收斂):**
  - §2.3 CJK：首選 `char_wb` + (2,3)-gram（Gemini Major 1，零依賴 multilingual），jieba 等改為備選
  - §5.1.1 Auto-accept：改 conservative（Gemini Major 2 無向比對 + ChatGPT Critical 2 relation_type 匹配 + confidence≥med）
  - §5.1.1 + parent §5.1 Class promotion：accept 時自動把 exploratory/proposed → canonical，保留 `promoted_from` 欄位（Gemini Major 3 + ChatGPT Critical 1 合成）
  - §2.3 Step 6 NetworkX：degree-0 孤島前置濾除，避免 Louvain 爆錯（Gemini P2）
  - §2.3 Step 7 Spaghetti：threshold 調為 `max(5, N/3)` 避免小網路誤判（Gemini P2）
  - §4.1 Output：加 Top 5 / Next 10 / Appendix 三層結構（ChatGPT Warning 3 attention protection）
...
**CJK tokenization（v1.2.2 採納 Gemini 更簡潔解法）：**

TF-IDF 預設 whitespace split 對 CJK content 會失真（整句變一個 token）。**首選解法**：用字元 n-gram 分詞，零依賴多語通吃：

```python
from sklearn.feature_extraction.text import TfidfVectorizer
# analyzer='char_wb': 在字元邊界切分，對中/日/韓/英皆生效
# ngram_range=(2,3): 2-3 字組合涵蓋中文詞彙重疊 + 英文短詞根
vectorizer = TfidfVectorizer(analyzer='char_wb', ngram_range=(2, 3))
```

**備選方案（若 char n-gram 精度不足）：** 用 `jieba`（中）/ MeCab（日）/ KoNLPy（韓）+ `langdetect` 先判語種。但對 Pass 1 快速預篩，char n-gram 準度已夠，且避免多 tokenizer + 語種偵測的依賴地獄。

**實作 gate：** Skill implementation 前必驗 Pass 1 對實際 HB whiteboard（多半中文）跑得對；若 baseline 對中文卡片 cosine similarity 全部趨近同值 → 確認 `analyzer='char_wb'` 有套對。
...
Step 6: Clustering proposal（v1.2 NetworkX offload + v1.2.2 孤島防護）
  - 以 proposed + existing links 為 graph edges
  - **不由 LLM 腦算 modularity**（不可靠）→ 寫 edge list + node list 到 `/tmp/propose_links_{ts}.json`
  - **v1.2.2 前處理（Gemini P2）**：在丟給 Louvain 之前，先把 `degree == 0` 節點移除
    （避免 `ZeroDivisionError` 或無意義分群），歸入 Gap Signal `weak_integration`
  - 剩下的 connected components 再跑 `networkx.algorithms.community.louvain_communities`
  - Python 輸出 JSON: `{clusters: [{nodes: [...], density: float}], isolated: [nodeIds...]}`
...
Step 7: Gap signal（v1.2 擴充至 5 類）
  ...
  - LLM 依 Python 算出的數字判定以下 5 類 gap signal：
    ...
    **🔹 Spaghetti warning**（v1.2 新增，Gemini G7；v1.2.2 threshold 調整）: 某卡 `degree > max(5, N/3)` — 概念過載 / 黑洞化。建議（1）抽象為高維原則另建卡、或（2）拆為 2-3 張子卡。**基準 5 的理由**：小型 whiteboard（N=12）連 4 條線是健康 hub，不該誤判為黑洞；絕對值 5 保證小型網路不被 over-warn
...
## Proposed New Links — Attention-Protected Hierarchy (v1.2.2, ChatGPT Warning 3)

為保護 user attention，不 dump 全部建議。固定三層結構：

### ⭐ Top 5 — Must-review high-signal links

（Top 5 選擇標準：confidence=high + novelty from ≥2 elevation anchors + 不 redundant；按 signal quality 排序）

### 📌 Next 10 — Optional medium-signal links

（中段：confidence=med 或高但某維度重疊；看有時間時檢查）

### 📎 Appendix — 其餘 low-confidence / speculative

（<details> 折疊；備查用，user 若覺得前面不夠豐富再展開；absolute top 50 pair cap 內剩餘全部放這）

### Confidence legend: 🟢 high / 🟡 med / 🔴 low
...
### 5.1.1 Auto-accept Detection（v1.2.2 強化 conservative rule，兩 AI review 收斂）

**問題 v1.1 未解：** `_discovered_links.json` 的 entries 若無人手動改 `acceptance_state`，會永遠停在 `proposed`，形成「proposed 死水」。人類不會去手改 JSON。

**v1.2 初版解法 + v1.2.2 conservative 強化：** 兩 AI review 同時警告 v1.2 initial 版太 aggressive（user 可能只是視覺 grouping 而非語意 accept；edge 可能有 direction 差異；相似但不完全相同的 edge 可能被誤讀）。v1.2.2 改為 **topology detection + conservative auto-accept**：

```python
# 流程（v1.2.2 pseudocode）
for link in registry.filter(
    scope_whiteboard_id == current_whiteboard_id,
    acceptance_state == "proposed"
):
    # Topology check: 無向比對（Gemini G2）
    # User 在 GUI 拉線時方向隨機，只要 (A,B) 或 (B,A) 之間有實體連結就算偵測到
    topology_match = (link.from, link.to) ∈ edges OR (link.to, link.from) ∈ edges

    if not topology_match:
        continue  # 不動此 link

    # 觀察到 topology 匹配 —— 但要不要升 accepted 分 3 級
    link.implicit_connection_detected = True
    link.last_verified_at = now()

    # Conservative auto-accept（ChatGPT Critical 2）:
    # 嚴格比對 relation_type 是否也匹配 + confidence 足夠
    same_relation = (existing edge 的 label / relation 標籤 與 link.relation_type 一致)
    confidence_enough = link.confidence ∈ {"high", "med"}

    if same_relation AND confidence_enough:
        # 完全確定 — auto-accept
        link.acceptance_state = "accepted"
        link.auto_accept_reason = "exact_pair_existing_connection_matching_relation"
        link.auto_accept_confidence = "high"
        link.verified_by = "mixed"
        # Class promotion（Gemini Major 3）: exploratory/proposed → canonical
        if link.link_class in ("exploratory", "proposed"):
            link.promoted_from = link.link_class
            link.link_class = "canonical"
    else:
        # 偵測到連結但規則不符 — 保守只 mark, 不升 state
        link.auto_accept_reason = (
            "exact_pair_existing_connection" if not same_relation
            else "low_confidence"
        )
        link.auto_accept_confidence = "low" if link.confidence == "low" else "medium"
        # acceptance_state 保持 "proposed"

    registry.update(link)
```

**新增 registry 欄位（對齊 v3 Schema v2 擴充）：**
```json
{
  "implicit_connection_detected": true,
  "auto_accept_reason": "exact_pair_existing_connection_matching_relation",
  "auto_accept_confidence": "high|medium|low",
  "promoted_from": "exploratory|proposed"  // 若 class 被晉升
}
```

**為什麼保守而不 aggressive：**
- User 在 GUI 拉線可能只是 visual grouping，不是語意 accept
- 既有 edge 可能只是 `related` 泛型，不等於 AI 提議的 relation_type
- 相似但不完全相同 edge 不該被當作接受訊號
- 保守規則可 user-audited（後續可以人工把 low-confidence 的 implicit 升級為 accepted）
